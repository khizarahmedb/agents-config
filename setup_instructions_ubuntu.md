...
This script renders repo files from canonical templates in `<LOCAL_CONFIG_REPO_PATH>/templates/repo`.
...
bash <LOCAL_CONFIG_REPO_PATH>/scripts/install_grounded.sh --scope global --agents all --with-global-git-template
...
```bash
git ls-files -z -- 'AGENT_NOTES*.md' '**/AGENT_NOTES*.md' '.agentsmd' '**/.agentsmd' | xargs -0 -r git rm --cached --ignore-unmatch
```
...
13. `bash <LOCAL_CONFIG_REPO_PATH>/scripts/validate_setup_consistency.sh` passes.
